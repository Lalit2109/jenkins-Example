...
function Get-BackupManifest {
    ...
    foreach ($repo in $Repositories) {
        $manifest.Repositories += @{
            ...
            BackupDate = $Date
            BlobPath = "$($repo.Project)/$($repo.Name)/$Date.zip"
        }
    }
...
try {
    ...
    foreach ($repo in $repos) {
        ...
        try {
            $url = ($repo.RemoteUrl).Substring(30)
            $url = "https://$AccessToken@$url"
    ...
    foreach ($r in $repoList) {
        try {
            ...
            $failureCount++
            Write-Log "Failed to backup $($r.OriginalProject)/$($r.OriginalName): $($_.Exception.Message)" "ERROR"
    ...
    try {
        $manifest = Get-BackupManifest -Repositories $processedRepos -Date $date
        $manifestJson = $manifest | ConvertTo-Json -Depth 10
        $manifestPath = ".\backup-manifest-$date.json"
        ...
        Set-AzStorageBlobContent @manifestBlobParams -ErrorAction Stop
        Remove-Item -Force -Path $manifestPath -ErrorAction SilentlyContinue
        Write-Log "Manifest uploaded successfully" "SUCCESS"
    } catch {
